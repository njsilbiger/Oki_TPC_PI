...
ggsave(here("Output", "Physiology", "dryweight_species_jitter.pdf"), dw_plot, h = 5, w = 10)
```
...
chla_7sp <- chla_avg %>% 
  filter(full_species %in% sp_keep) %>%
  filter(frag_ID != "E04" & frag_ID != "E05") #removing exceptionally high chla/sym samples
...
#| fig.width: 15
#| fig.height: 15
physio_plots <- ggarrange(chla_plot, chla_sym_plot, sym_plot, dw_plot, prot_plot, nrow = 3, ncol = 2, legend = "right", common.legend = TRUE)
