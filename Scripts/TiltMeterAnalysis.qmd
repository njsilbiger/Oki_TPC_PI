...
#Load Meta Data
TiltMetaData <- write.csv(here("Data/TiltMeterData/MetaData.csv"))
TiltMetaData <- read_csv(here("Data/TiltMeterData/MetaData.csv"))
LatLong <- TiltMetaData %>% dplyr::select(Site, Long, Lat)
...
mutate( #Set datetime,a dding in the UTC timezone so wer are removing taht
    ...
    dplyr::select(TiltMeter, everything()) #Moving TiltMeter# Column to be the first column
...
separate(col = DateTime, #Choose the column that you want separated
  ...
  mutate(DateTime = lubridate::ymd_hms(DateTime), #Set datetime
   ...
   TiltMeter = tilt_name, na.rm = TRUE) %>%
    ...
    dplyr::select(TiltMeter, everything()) 
...
MeansFlow <- Means %>% rename(SiteName = Site)
write.csv(MeansFlow, here("Data/SfM/FlowMetrics.csv"), row.names = FALSE)
...
TempMeans <- JoinedTempData %>%
  ...
  summarise(minTemp = min(Temperature),maxTemp =  max(Temperature), meanTemp = mean(Temperature)) %>%
  mutate_if(is.numeric, round, 2)

Temp <- TempMeans %>% rename(SiteName = Site)
write.csv(Temp, here("Data/SfM/TempMeans.csv"), row.names = FALSE)
