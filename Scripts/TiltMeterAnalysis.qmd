...
Load current, temperature, and tide data from new csv files created.
...
LatLong <- TiltMetaData %>% select(Site, Lat, Long)
...
JoinedData <- JoinedData %>% 
  mutate(Site = case_when(
    str_detect(TiltMeter, "Tilt1") ~ "Oki 2", 
    str_detect(TiltMeter, "Tilt2") ~ "Oki 3", 
    str_detect(TiltMeter, "Tilt3") ~ "Oki 4", 
    str_detect(TiltMeter, "Tilt4") ~ "Oki 7", 
    str_detect(TiltMeter, "Tilt5") ~ "Oki 9", 
    str_detect(TiltMeter, "Tilt6") ~ "Oki 11", 
    str_detect(TiltMeter, "Tilt7") ~ "Oki 15", 
    str_detect(TiltMeter, "Tilt8") ~ "Oki 14" )) %>%
      relocate(Site, .after = TiltMeter) #Move new site column to after the Tilt Meter
  

```

Visualize Summary stats to see where to filter data from
...
Means<- JoinedData %>%
  ...
  arrange(MeanSpeed) #Reorder so that the means are from lowest ot highest speed

   kbl(Means, col.names = c("Site", "Mean Speed (cm/s)")) %>% #Giving the average speed m/s of each tilt meter
  kable_classic() %>%
...
Means\<- JoinedData %\>% group_by(Site) %\>% filter(Speed \<= 25.01416) %\>% summarise(MeanSpeed = mean(Speed, na.rm = TRUE)) %\>% arrange(MeanSpeed) #Reorder so that the means are from lowest ot highest speed

kbl(Means, col.names = c("Site", "Mean Speed (cm/s)")) %\>% #Giving the average speed m/s of each tilt meter kable_classic() %\>% kable_styling(full_width = FALSE)
...
JoinedTempData <- JoinedTempData %>% 
  mutate(Site = case_when(
    str_detect(TiltMeter, "Tilt1") ~ "Oki2", 
    str_detect(TiltMeter, "Tilt2") ~ "Oki3", 
    str_detect(TiltMeter, "Tilt3") ~ "Oki4", 
    str_detect(TiltMeter, "Tilt4") ~ "Oki7", 
    str_detect(TiltMeter, "Tilt5") ~ "Oki9", 
    str_detect(TiltMeter, "Tilt6") ~ "Oki11", 
    str_detect(TiltMeter, "Tilt7") ~ "Oki15", 
    str_detect(TiltMeter, "Tilt8") ~ "Oki14" )) %>%
      relocate(Site, .after = TiltMeter) %>% #Move new site column to after the Tilt Meter
 left_join(TiltMetaData, select(Lat, Long), by = "Site")
...
This code is converted from Matlab Katie Smith Via Christina Comfort First thing we have to do is make a function for the prax_ellipse
...
TiltEllipses <- JoinedData %>% 
  ...
  summarise( 
    ...
    phi_deg <- 90 - (results$phi * 180 / pi) %% 360 
    phi_deg <- ((phi_deg + 180) %% 360) -180 #Try to not to add 180 
